...
function createComment(url, request){
  const req_comment = request.body && request.body.comment;
  const response = {};

  if (req_comment && req_comment.body && req_comment.articleId && 
    database.articles[req_comment.articleId] && req_comment.username && database.users[req_comment.username]){
      const comment = {
...
function updateComment(url, request){
  ...
  if(!id || !requestComment){
    ...
    response.body = {updatedComment: savedComment};
    response.status = 200;
  }
  return response;
}
...
function upvoteComment (url, request){
  ...
  const comment = database.comments[commentId];
  const response = {};
  if (!(commentId && comment && database.users[username])){
    response.status = 400;
    return response;
  }
  const commentUpvotes = comment.upvotedBy;
  const commentDownvotes = comment.downvotedBy;
  if(!commentUpvotes.includes(username)){
    if (commentDownvotes.includes(username)){
      commentDownvotes.splice(commentDownvotes.indexOf(username),1)
    }
    commentUpvotes.unshift(username)
    response.body = {comment: comment};
    response.status = 200;
  }else{
    response.status = 400;
...
function downvoteComment (url, request){
  ...
  if(!commentDownvotes.includes(username)){
    if (commentUpvotes.includes(username)){
      commentUpvotes.splice(commentUpvotes.indexOf(username),1)
    }
    commentDownvotes.unshift(username)
    response.body = {comment: comment};
    response.status = 200;
  }else{
    response.status = 400;
